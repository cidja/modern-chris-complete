      ...
      for(let stations of contrats) {
       ...
       //Condition pour afficher les fleches de couleur en fonction des résultats
          if(status === "OPEN"){ //Si status station open 
            ...
            else if((velosDisponible <= 3) & (velosDisponible > 0)) { //Sinon on affiche icon orange si inférieur à 3vélos et supérieur à 0
...
//        Bloc réservation de la station
  ...
  initRerservationListener(){
    $('#validationbouton').on('click',  () =>{ //conditions pour pouvoir lancer la réservation
      ...
      else if(canvas.toDataURL() == document.getElementById("blank").toDataURL()){ //source: https://jsfiddle.net/amaan/rX572/
          $("#signmessage").text("Merci de signer");
          setTimeout(function () {
            $('#signmessage').hide(1000); //cache test
        },2000);       
       }
     else{
