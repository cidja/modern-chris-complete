...
namespace cidja\velomanager; //source: https://youtu.be/WHtbi8S0rkI?t=163
...
class Model_veloManager extends Model_ManagerDb
{
    public function allvelo()
    {
        $db =$this->dbConnect();
        $infos = $db->query('SELECT id, DATE_FORMAT(date_travel, "%d/%m/%Y à %Hh%imin%ss") AS date_travel,km FROM velo ORDER BY id DESC');
        return $infos;
    }
}
/* 
